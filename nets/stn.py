...
def spatial_transformer_network(input_fmap, scope="", out_dims=None, **kwargs):
    """
    ...
    theta_init = theta_init.astype('float32')
    theta_init = theta_init.flatten()
    loc_in = B * H * W
    loc_out = 6
    # W_loc = tf.Variable(tf.zeros([loc_in, loc_out]), name='W_loc')
    W_loc = tf.zeros([loc_in, loc_out])
    b_loc = tf.Variable(initial_value=theta_init, name='b_loc')
    theta = tf.matmul(tf.zeros([B, loc_in]), W_loc) + b_loc
